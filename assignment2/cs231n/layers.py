    ...
    for n in range(N):
        for hh in range(H_out):
            for ww in range(W_out):
                h_start = stride * hh
                w_start = stride * ww
                cur_x = x_pad[n, :, h_start:h_start + HH, w_start:w_start + WW]

                for f in range(F):
                    out[n, f, hh, ww] = np.sum(cur_x * w[f, :, :, :]) + b[f]
    ...
    for n in range(N):
        for hh in range(H_out):
            for ww in range(W_out):
                ...
                w_start = stride * ww
                cur_x = x_pad[n, :, h_start:h_start + HH, w_start:w_start + WW]
                cur_dx = dx_pad[n, :, h_start:h_start + HH, w_start:w_start + WW]

                for f in range(F):
                    dw[f, :, :, :] += cur_x * dout[n, f, hh, ww]
                    cur_dx += w[f, :, :, :] * dout[n, f, hh, ww]
        ...
        for c in range(C):
            for h in range(H_out):
                for w in range(W_out):
                    ...
                    out[n, c, h, w] = np.max(x[n, c, h_start:h_end, w_strat:w_end])
        ...
        for c in range(C):
            for h in range(H_out):
                for w in range(W_out):
                    ...
                    cur_x = x[n, c, h_start:h_end, w_strat:w_end]
                    cur_max = cur_x == np.max(cur_x)
                    dcur_x = cur_max * dout[n, c, h, w]

                    dx[n, c, h_start:h_end, w_strat:w_end] += dcur_x
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    for g in range(G):
        ...
        cur_x = x[:, start:end, :, :]
        cur_x = cur_x.reshape(N, -1)
        cur_gamma = np.ones(cur_x.shape[1])
        cur_beta = np.zeros(cur_x.shape[1])

        cur_out, cur_cache = layernorm_forward(cur_x, cur_gamma, cur_beta, gn_param)
        out[:, start:end, :, :] = cur_out.reshape((N, group_size, H, W))
        cache.append(cur_cache)

    cache = (cache, out, G, gamma)
    out = out * gamma + beta
...
def spatial_groupnorm_backward(dout, cache):
    ...
    for g in range(G):
        ...
        cur_dout_ = dout_[:, start:end, :, :]
        cur_dout_ = cur_dout_.reshape(N, -1)
        cur_cache = cache[g]

        cur_dx, _, _ = layernorm_backward(cur_dout_, cur_cache)
        dx[:, start:end, :, :] = cur_dx.reshape((N, group_size, H, W))
